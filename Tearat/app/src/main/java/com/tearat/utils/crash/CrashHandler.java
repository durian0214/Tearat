...
public class CrashHandler implements Thread.UncaughtExceptionHandler {
    ...
    private String bugUrl, throwableStr;
    ...
    public void init(Application application, boolean isDebug, String bugUrl) {
        this.bugUrl = bugUrl;
        initCrashHandler(application, isDebug);
    }
...
//
    ...
    private boolean handleException(Throwable ex) {
        ...
        throwableStr = ex == null ? "null" : ex.toString();
        Log.e(TAG, "mIsDebug:" + mIsDebug + ",Throwable:" + throwableStr);
        if (ex == null) {
            return false;
        }
        if (mIsDebug) {
...
//        Log.e(TAG, "saveCatchInfo2File start");
...
//            Log.e(TAG, "key:" + key + ",value:" + value);
        ...
        try {
            ...
            if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
                ...
                fos.close();
                //保存到sp
                new CrashUtil().savaCrash(mApplication, new CrashBean(infos.get(CrashBean.STR_versionCode),
                        infos.get(CrashBean.STR_versionName), infos.get("TYPE"),
                        "MODEL", infos.get("TIME"),
                        infos.get("BRAND"), infos.get("CPU_ABI"),
                        ex.getMessage(), path + fileName), false);
            }
            return fileName;
        } catch (Exception e) {
            e.printStackTrace();
...
//            Log.e(TAG, "LogcatCrashInfo() 日志文件不存在");
    ...
    public void postCrash(final String bugUrl) {
        Map<String, Object> map = new HashMap<>();
        map.put("versionCode", infos.get("versionCode"));
        map.put("versionName", infos.get("versionName"));
        map.put("type", infos.get("TYPE"));
        map.put("model", infos.get("MODEL"));
        map.put("time", infos.get("TIME"));
        map.put("brand", infos.get("BRAND"));
        map.put("cpu_abi", infos.get("CPU_ABI"));
        String exception = "";
        try {
            exception = NslmCryptoUtils.encrypt(throwableStr);
        } catch (Exception e) {
            e.printStackTrace();
        }
        map.put("exception", exception);
        HttpUtils.getInstance().jsonPost(bugUrl, map, new HttpCallback() {
            @Override
